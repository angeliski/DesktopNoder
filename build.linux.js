...
var init = function () {
    projectDir = jetpack;
    //caminho onde o arquivo gerado vai ser salvo
    tmpDir = projectDir.dir('./dist', { empty: true });
    //caminho do que vai ser "empacotado"	
    releasesDir = projectDir.dir('./build');
    manifest = releasesDir.read('./package.json', 'json');

    packName = manifest.name + '_' + manifest.version;
    packDir = tmpDir.dir(packName);
    //pasta onde o arquivo vai ser gerado
    readyAppDir = packDir.cwd('linux', manifest.name);
